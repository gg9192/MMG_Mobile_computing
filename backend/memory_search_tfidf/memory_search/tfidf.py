...
def getMemorySubset(query:str, memories:list, n:int) -> list:
    ...
    for i in range(0, len(memories)):
        ...
        if math.isnan(cossim):
            # sometimes the cosine similarity is NAN, the way to get this
            # to happen is to have 1 query word that got returned from yake,
            # and the memory does not have this single word. In that case, it's
            # 100% unrelated so make it's value -1. Recall that cosine sim is 
            # from -1 to 1 inclusive. just a weird edge case. 
            res.append((-1, memories[i]))
        else:
